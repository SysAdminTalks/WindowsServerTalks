...
document.addEventListener('DOMContentLoaded', () => {
  ...
  injectPartial('#sidebar-container', '/assets/includes/sidebar.html', (sidebarHost) => {
    ...
    const nav = sidebarHost.querySelector('.sidebar') || sidebarHost; // robust if wrapper changes
    const heading = nav.querySelector('h3');
    const list = nav.querySelector('ul');
    if (!heading || !list) return; // defensive guard
    ...
    // Highlight helper (wrap match in <mark>)
    const highlight = (el, needle) => {
      if (!el || !el.textContent) return;
      const text = el.textContent;
      if (!needle) { el.innerHTML = text; return; }
      const i = text.toLowerCase().indexOf(needle.toLowerCase());
      if (i === -1) { el.innerHTML = text; return; }
    ...
    const filter = (q) => {
      ...
      items.forEach(({ li, link, lower }) => {
        const hit = !needle || lower.includes(needle);
        li.style.display = hit ? '' : 'none';
        if (link) {
          highlight(link, needle);
          if (hit && !firstVisibleLink) firstVisibleLink = link;
        }
      });
